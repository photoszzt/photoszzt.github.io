...
- Model:
  ...
  - State transition probability: In state s, when taking action a, the probability of transitioning to state s′ is p(s'|s, a). It holds that

    $$\sum_{s′ \in S} p(s′|s, a) = 1$$ for any (s, a).

  - Reward probability: In state s, when taking action a, the probability of obtaining reward r is p(r|s, a). It holds that
