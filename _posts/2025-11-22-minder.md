...
- Analytics:
  ...
  5. Fed the normalized data to the corresponding metric LSTM-VAE model to get the embedding. 
  6. Calculate pairwise Euclidean distances of embeddings between every two machines. 
  7. For each machine, get the sum of the distances to other machines
  8. Calculate the normal/z-score of the sum of the distances of each machine. avg(x) is the average of x and std(x) is the standard deviation of x. 
      $$z =\frac{x - avg(x)}{std(x)}$$ 
  9. The machine with the maximum normal score is probably the faulty one. If the maximum normal score is higher than a similarity threshold, the machine is assumed as a candidate of the time window
  10. Continuity check: shifts the time window with one data sample to detect the potentially faulty machine for new windows. If the same machine is detected with consecutive times that exceed a continuity threshold, 4mins, it is considered a truly faulty machine. 
